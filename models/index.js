...
let config = require(__dirname + "/../db/config.js")[env];
let db = {};
let sequelize = null;

if (process.env.JAWSDB_URL) {
  console.log("Starting JAWSDB");
  sequelize = new Sequelize(process.env.JAWSDB_URL);
} else if (config.use_env_variable) {
  console.log("Starting DB with use_env_variable");
  sequelize = new Sequelize(process.env[config.use_env_variable]);
} else {
  console.log(`Starting DB with config values ${config.database}`);
...
fs
  ...
  .forEach(function(file) { // for each .js file, call "import" method
    console.log("Sequelize import of " + file);
